...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-navy/90 backdrop-blur-md py-4 shadow-lg' : 'bg-transparent py-6'}`}>
      <div className="container max-w-6xl mx-auto px-4 sm:px-6 flex justify-between items-center">
        ...
        {/* Mobile menu button */}
        <button onClick={toggleMenu} className="md:hidden text-slate-light hover:text-highlight transition-colors">
        ...
        <nav className="hidden md:flex gap-8">
          {navItems.map((item, index) => (
            ...
            >
              <span className="text-highlight text-sm mr-1">0{index + 1}.</span> {item.name}
            </a>
          ))}
          <Button className="ml-4 bg-transparent hover:bg-highlight/10 text-highlight border border-highlight rounded transition-all">
            Resume
          </Button>
        </nav>
